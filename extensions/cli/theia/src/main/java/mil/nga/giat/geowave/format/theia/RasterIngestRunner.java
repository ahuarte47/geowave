		...
		DownloadRunner
{
	...
	private static final double THEIA_NO_DATA_VALUE = 0;

	private static Map<String, TheiaBandConverterSpi> registeredBandConverters = null;
	protected final List<String> parameters;
	protected TheiaRasterIngestCommandLineOptions ingestOptions;
	...
	protected BandData getBandData(
		...
		final String coverageName = FreeMarkerTemplateUtils.processTemplateIntoString(
				...
				model);
		final File geotiffFile = DownloadRunner.getCoverageFile(
				band,
		...
		if ((ingestOptions.getCoverageConverter() != null) && !ingestOptions.getCoverageConverter().trim().isEmpty()) {
			...
			final TheiaBandConverterSpi converter = getConverter(ingestOptions.getCoverageConverter());
			if (converter != null) {
		...
		if (ingestOptions.isCropToSpatialConstraint()) {
			...
			final Filter filter = theiaOptions.getCqlFilter();
			if (filter != null) {
		...
		if (!ingestOptions.isCoveragePerBand()) {
			...
			if (!lastSceneBands.isEmpty()) {
				...
				if (sceneData.size() == 1) {
					...
					for (final BandData bandData : sceneData.values()) {
	...
	public TheiaBandConverterSpi getConverter(
			final String converterName ) {
		final TheiaBandConverterSpi converter = getRegisteredConverters().get(
				converterName);
		if (converter == null) {
			LOGGER.warn("no Theia converter registered with name '" + converterName + "'");
		}
		return converter;
	}

	private synchronized Map<String, TheiaBandConverterSpi> getRegisteredConverters() {
		if (registeredBandConverters == null) {
			registeredBandConverters = new HashMap<String, TheiaBandConverterSpi>();

			final ServiceLoader<TheiaBandConverterSpi> converters = ServiceLoader.load(TheiaBandConverterSpi.class);
			final Iterator<TheiaBandConverterSpi> it = converters.iterator();
			while (it.hasNext()) {
				final TheiaBandConverterSpi converter = it.next();
